...
- Run `uv run pytest backend/tests -q --ignore=backend/tests/test_seeding.py` before considering any task complete
...
- `backend/tests/test_plan.md` — update test coverage status
- This file (`Agents.md`) — if execution conventions changed

## 4. Code Conventions

- CLI commands use `click` and live in `backend/omnifin/cli/`
- Domain models use Pydantic v2 and live in `backend/omnifin/models/`
